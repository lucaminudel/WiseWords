...
- When technical decisions on how to solve a problem have multiple valid approaches and options, present the optionns and ask to discuss them before proceeding.
- Complete one layer fully before moving to the next, therefore following an incremental iterative approach.
- Before making any changes to the code, ask permisison to analyse the current implementation first.
